    ...
    <div>
      ...
      <div className="mt-1 relative rounded-md shadow-sm">
        <div className="absolute inset-y-0 left-0 pl-3 flex items-center">
          <div>
            <button onClick={(() => setEquality(equality + 1 > Object.keys(equalityEnum).length ? 1 : equality + 1) )} className="flex items-center justify-center p-1 focus:outline-none">
              {equality === equalityEnum.equal && 
                <svg className="h-4 w-4 text-gray-400" aria-hidden="true" focusable="false" data-prefix="fal" data-icon="equals" role="img" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 384 512">
        ...
        <div className="absolute inset-y-0 right-0 pr-3 flex items-center">
          <div>
            <button onClick={() => updateFilterQuery({key: propertyName, value: thisFilterValue, equality, propertyName})} className="flex items-center justify-center p-1 focus:outline-none">
